...
class DataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto) -> Dict[str, Any]:
        ...
        if len(data) > self.config.global_batch_size_per_device:
            data.truncate(self.config.global_batch_size_per_device)
        mini_batches = data.select(select_keys, non_tensor_select_keys).split(min(self.config.global_batch_size_per_device, len(data)))
