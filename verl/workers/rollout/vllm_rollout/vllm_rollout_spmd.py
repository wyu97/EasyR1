...
class vLLMRollout(BaseRollout):
    def __init__(self, model_path: str, config: RolloutConfig, tokenizer: PreTrainedTokenizer):
        ...
        if config.limit_images > 0:
            vllm_init_kwargs = {"limit_mm_per_prompt": {"image": config.limit_images}, "mm_processor_kwargs": {"max_pixels": 4194304}}
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if not do_sample:
            print ('switching to greedy decoding')
            kwargs = {
                "n": 1,
                "temperature": 0.0,
                "top_p": 1.0,
        ...
        if "multi_modal_data" in non_tensor_batch:
            ...
            for raw_prompt_ids, multi_modal_data in zip(
                ...
                print ('prompt_token_ids in vllm generation', len(raw_prompt_ids))
        else:
            vllm_inputs = [
        ...
        print (len(completions[0].prompt_token_ids))
        response_ids = []
    ...
    @torch.no_grad()
    def generate_sequences_step(self, prompts: DataProto, **kwargs) -> DataProto:
        # left-padded attention_mask
        input_ids: torch.Tensor = prompts.batch["input_ids"]  # (bs, prompt_length)
        attention_mask: torch.Tensor = prompts.batch["attention_mask"]
        position_ids: torch.Tensor = prompts.batch["position_ids"]
...
if __name__ == '__main__':
    ...
    os.environ["MASTER_ADDR"] = "29.81.224.173"
    os.environ["MASTER_PORT"] = "19000"
    vllm_init_kwargs = {"limit_mm_per_prompt": {"image": 1}}
    inference_engine = LLM(
            model='/apdcephfs_sh2_300000800/share_300000800/models/Qwen2.5-VL-7B-Instruct/',
            skip_tokenizer_init=False,
    ...
    sampling_kwargs = {"max_tokens": 2048, "detokenize": False, }
    default_sampling_params = SamplingParams()
    # for key in config.to_dict().keys():
    #     if hasattr(default_sampling_params, key):
    #         sampling_kwargs[key] = getattr(config, key)
    ...
    dummy_image = dummy_image.resize((dummy_image.width//2, dummy_image.height//2), Image.Resampling.LANCZOS)
    print (dummy_image.height, dummy_image.width)
    agent = Qwen25VLAgent(policy_lm='/apdcephfs_sh2_300000800/share_300000800/models/Qwen2.5-VL-7B-Instruct', max_prompt_len=8192)
    observation = {'history': [], 'task': 'dummy task', 'image': dummy_image}
    prompts = agent.get_action_inputs([observation])
    print(f"Sampling params: {sampling_kwargs}.")
    sampling_params = SamplingParams(**sampling_kwargs)
    vllm_inputs = []
    for raw_prompt_ids, multi_modal_data in zip(
        prompts.non_tensor_batch.pop("raw_prompt_ids"), prompts.non_tensor_batch.pop("multi_modal_data")
    ):
        vllm_inputs.append({"prompt_token_ids": raw_prompt_ids, "multi_modal_data": multi_modal_data})
    results = inference_engine.generate(
                prompts=vllm_inputs, sampling_params=sampling_params
            )
    print (len(results[0].prompt_token_ids))