...
class DataProto:
    ...
    def truncate(self, chunks: int) -> List["DataProto"]:
        """Truncate the data by dropping last few examples
        """
        num_to_drop = len(self) % chunks 
        if num_to_drop > 0:
            print ('before truncate', self.batch.shape)
            self.batch = self.batch[:-num_to_drop]
            print ('after truncate', self.batch.shape)

            for key, value in self.non_tensor_batch.items():
                assert isinstance(value, np.ndarray)
                self.non_tensor_batch[key] = value[:-num_to_drop]
